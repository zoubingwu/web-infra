...
// @ts-ignore
import mdx from '@mdx-js/mdx' // FIXME: mdx-2.0.0-next.9 missing some types, upgrade it as soon as newer version released
...
DOCS_ENTRY.forEach(doc => {
  ...
  const extractTitle = () => tree => visit(tree, 'heading', (node) => {
        ...
        if (node.depth === 1) {
          ...
          if (parentTitle) {
            children.push(createItem(value, node.depth, parentTitle + '.' + toRoute(value)))
          } else {
            children.push(createItem(value, node.depth, toRoute(value)))
          }
        }
...
fs.writeFileSync(path.resolve(SRC_DIR, './docs.generated.json'), JSON.stringify(docsData, null, 2))
