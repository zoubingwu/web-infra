...
With a proper formatter running automatically and intergrated to our workflow, it removes style related distractions when writing code, every single developper can write code in their own way they like, using tabs or spaces, semis or no semis, working on a extremely complex problem and just want to be focus so put everything all on one line, those are totally find since eventually everything will be instantly formatted when commit it to the code base.
